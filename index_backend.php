...
if ($_POST['form_name'] == 'to_sign') {
    ...
    if (empty($name) || empty($mail) || empty($password) || empty($repeated_password) || $password != $repeated_password) {
        ...
        if (empty($name)) {
            $sign_msg['sign_username_error_msg'] = $error_empty_msg;
        }
        if (empty($mail)) {
            $sign_msg['sign_mail_error_msg'] = $error_empty_msg;
        }
        if (empty($password)) {
            $sign_msg['sign_pwd_error_msg'] = $error_empty_msg;
        }
        if (empty($repeated_password)) {
            $sign_msg['sign_repeat_pwd_error_msg'] = $error_empty_msg;
        }
        if ($password != $repeated_password) {
            $sign_msg['sign_repeat_pwd_error_msg'] = "Password confirmation and Password must match.";
        }
...
if ($_POST['form_name'] == 'to_log') {
    ...
    if (empty($uname_or_mail) ||  empty($password)) {
        ...
        if (empty($uname_or_mail)) {
            $log_msg['log_uname_or_mail_error_msg'] = $error_empty_msg;
        }
        if (empty($password)) {
            $log_msg['log_pwd_error_msg'] = $error_empty_msg;
        }
